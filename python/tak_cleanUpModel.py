...
def UI():
    ...
    cmds.columnLayout('procColLay', adj = True)
     ...
     # Instance to Object
    # cmds.button(label = 'Instance to Object', c = uninstance)
    ...
    # go out to the 'procColLay' Layout
    cmds.setParent('procColLay')
    cmds.separator(h = 5, style = 'in')
    ...
    cmds.rowColumnLayout('symRowColLay', numberOfColumns = 4, columnWidth = [(1, 100), (2, 100), (3, 50)], columnSpacing = [(2, 5), (3, 5)])
    cmds.button('symChkButton', label = 'Check Symmetry', c = checkSym)
    cmds.button('makeSymButton', label = 'Make Symmetry', c = makeSym)
    cmds.floatField('ctVtxTolFltFld', v = 0.001, enable = False)
...
def makeSym(*args):
    ...
    # origMesh = cmds.duplicate(selLs[0], n = selLs[0] + '_origUV')
    # cmds.select(origMesh, r = True)
    # tak_misc.Wire()
